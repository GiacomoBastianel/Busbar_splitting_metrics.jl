...
test_case_file = joinpath(dirname(input_folder),"test_cases/cigre_b4_dc_grid.m")
test_case = _PM.parse_file(test_case_file)

s = Dict("output" => Dict("duals" => true))
result_opf = _PMACDC.run_acdcopf(test_case,ACPPowerModel,ipopt,setting=s)
result_opf_lpac = _PMACDC.run_acdcopf(test_case,LPACCPowerModel,gurobi,setting=s)
...
duals = []
for (b_id,b) in test_case["bus"]
    push!(duals,[parse(Int64,b_id),abs(result_opf_lpac["solution"]["bus"][b_id]["lam_kcl_r"])])
end
...
results_bs = _PMTP.run_acdcsw_AC_big_M(test_case_bs,formulation,optimizer)
