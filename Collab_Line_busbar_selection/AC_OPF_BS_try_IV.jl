...
test_case_file = joinpath(dirname(dirname(input_folder)),"test_cases/pglib_opf_case118_ieee.m")
...
println("Benefits from splitting busbar 49 is $((result_opf["objective"] - result_opf_check["objective"])), $((result_opf["objective"]-result_opf_check["objective"])/(result_opf["objective"])*100) %")
...
delete!(test_case_bs_check["switch"],"1")
delete!(test_case_bs_check["switch"],"2")
delete!(test_case_bs_check["switch"],"3")
delete!(test_case_bs_check["switch"],"4")
delete!(test_case_bs_check["switch"],"5")


prepare_AC_grid_feasibility_check(results_bs,test_case_bs_check_auxiliary,test_case_bs_check,switch_couples_feas_check,extremes_ZILs_ac,test_case)
result_opf_check = _PMACDC.run_acdcopf(test_case_bs_check,ACPPowerModel,ipopt; setting = s)

println("Benefits from splitting busbar 69,103,32,77,59 is $((result_opf["objective"] - result_opf_check["objective"])), $((result_opf["objective"]-result_opf_check["objective"])/(result_opf["objective"])*100) %")
