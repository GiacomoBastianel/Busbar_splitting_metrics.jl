...
function split_one_bus_per_time(test_case,results_dict,results_dict_ac_check)
    for (b_id,b) in test_case["bus"]
        ...
        if !isnan(results_dict["$b_id"]["objective"])
            ...
            result_opf_check["$b_id"] = _PMACDC.run_acdcopf(test_case_bs_check,ACPPowerModel,ipopt; setting = s)
        end
    end
end
result_bs_ii = Dict{String,Any}()
results_ac_check = Dict{String,Any}()
split_one_bus_per_time(test_case,result_bs_ii,results_ac_check)

for (b_id,b) in test_case["bus"]
    println([b_id,results_ac_check[b_id]["objective"]])
end

obj = result_opf["objective"]
