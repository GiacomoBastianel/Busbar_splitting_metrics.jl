...
input_folder = "/Users/giacomobastianel/Library/CloudStorage/OneDrive-KULeuven/data_sources/RTS-GMLC/RTS_Data/FormattedData/MATPOWER"

# Belgium grid without energy island
test_case_file = joinpath(input_folder,"RTS_GMLC.m")
...
for (b_id,b) in test_case["branch"]
    if b_id == "54"
        println("f_bus is $(b["f_bus"]), t_bus is $(b["t_bus"])")
    end
end
...
for (b_id,b) in test_case_bs_try["bus"]
    ...
    println("----------------")
    results_bs_hourly = _PMTP.run_acdcsw_AC_big_M_ZIL_no_OTS(hourly_test_case,formulation,optimizer)
